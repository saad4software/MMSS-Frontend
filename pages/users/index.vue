          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                v-bind="attrs"
                v-on="on"
                @click="$router.push(localePath('/users/0'))"
              >
                {{$t('New_Item')}}
              </v-btn>
            </template>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios.$get('users/?page_size='+itemsPerPage +'&page='+page+'&search=' + this.search)
        .then(response => {
    ...
    editItem (item) {
      ...
      this.$router.push(this.localePath('/users/' + this.editedItem.id))
    },
