          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                v-bind="attrs"
                v-on="on"
                @click="$router.push(localePath('/transactions/0'))"
              >
                {{$t('New_Item')}}
              </v-btn>
            </template>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios.$get('transactions/?min_date='+this.date_from+'&max_date='+this.date_to+'&page_size='+itemsPerPage +'&page='+page+'&search=' + this.search)
        .then(response => {
    ...
    editItem (item) {
      ...
      this.$router.push(this.localePath('/transactions/' + this.editedItem.id))
    },
    ...
    deleteItemConfirm () {
      this.$axios.$delete('transactions/'+this.editedItem.id)
        .then(response => {
